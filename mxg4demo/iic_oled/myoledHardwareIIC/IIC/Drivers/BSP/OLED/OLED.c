...
{
    uint8_t buffer[2];
    buffer[0] = 0x00;   // 控制字节：Co=0, D/C#=0 (写命令)
    buffer[1] = cmd;
...
{
    uint8_t buffer[2];
    buffer[0] = 0x40;   // 控制字节：Co=0, D/C#=1 (写数据)
    buffer[1] = data;
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, buffer, 2, HAL_MAX_DELAY);
}

// 批量写数据 (比如一行像素)
void OLED_WriteDataBuffer(uint8_t *data, uint16_t size)
{
    uint8_t buffer[size + 1];
    buffer[0] = 0x40;   // 控制字节
    memcpy(&buffer[1], data, size);
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, buffer, size + 1, HAL_MAX_DELAY);
}
...
void OLED_Update(void)
{
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteDataBuffer(OLED_DisplayBuf[j], 128);
	}
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteDataBuffer(&OLED_DisplayBuf[j][X], Width);
		}
	}
}
