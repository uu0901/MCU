...
#define LCD_SCLK(x)    do{HAL_GPIO_WritePin(SCLK_GPIO_Port,SCLK_Pin,(x) ? GPIO_PIN_SET :GPIO_PIN_RESET);}while(0)//SCL=SCLK

#define LCD_MOSI(x)    do{HAL_GPIO_WritePin(MOSI_GPIO_Port,MOSI_Pin,(x) ? GPIO_PIN_SET :GPIO_PIN_RESET);}while(0)//SDA=MOSI

#define LCD_RES(x)     do{HAL_GPIO_WritePin(RES_GPIO_Port,RES_Pin,(x) ? GPIO_PIN_SET :GPIO_PIN_RESET);}while(0)//RES

#define LCD_DC(x)      do{HAL_GPIO_WritePin(DC_GPIO_Port,DC_Pin,(x) ? GPIO_PIN_SET :GPIO_PIN_RESET);}while(0)//DC

#define LCD_BLK(x) 	   do{HAL_GPIO_WritePin(BLK_GPIO_Port,BLK_Pin,(x) ? GPIO_PIN_SET :GPIO_PIN_RESET);}while(0)//BLK
