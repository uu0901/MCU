...
volatile uint8_t WS2812B_Flag;					// 用来标记传输是否完成
/*
	...
	WS2812B_Bit[24*WS2812B_LED_QUANTITY+1];前 24 个元素 → 代表 24bit 的颜色数据
    ...
    for(j=0;j<WS2812B_LED_QUANTITY;j++) // 遍历每个 LED
    {
        for(i=0;i<24;i++) // 遍历每一位 每个 LED 的颜色用 24 位二进制表示，逐位判断输出 PWM 是“高占空比”还是“低占空比”。
        {
            ...
            if(WS2812B_Buf[j] & (0x800000 >> i)) // 判断当前 bit 是否为1
                WS2812B_Bit[j*24+i+1] = CODE_1;	 // “1” → 占空比大（~0.83µs）	
            else
                WS2812B_Bit[j*24+i+1] = CODE_0; 	 // “0” → 占空比小（~0.42µs）
        }
    }
    HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_1, (uint32_t*)WS2812B_Bit, 24*WS2812B_LED_QUANTITY+1);
...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
        __HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_1, 0); // 确保输出低电平，进入复位时间
...
void ColorRGB(uint16_t delay)
{
	...
	// 设置所有LED为当前颜色
	WS2812B_SetBuf(colors[color_index].value);
	WS2812B_UpdateBuf();
	...
	// 循环到下一个颜色
	color_index = (color_index + 1) % getColorCount();
}
