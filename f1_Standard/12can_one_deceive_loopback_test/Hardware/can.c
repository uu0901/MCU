...
void MyCAN_Init(void)
{
	...
	*/
	...
	CAN_FilterInitStructure.CAN_FilterNumber = 0;	//初始化过滤器几，共有14个过滤器，值为0~13
	CAN_FilterInitStructure.CAN_FilterIdHigh = 0x0000;
	CAN_FilterInitStructure.CAN_FilterIdLow = 0x0000;
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh = 0x0000;
	CAN_FilterInitStructure.CAN_FilterMaskIdLow = 0x0000;//32位屏蔽模式下 4个全给0就是全通
	...
	CAN_FilterInitStructure.CAN_FilterFIFOAssignment = CAN_Filter_FIFO0;//过滤器关联，通过此过滤器的报文，进FIFO0排队还是FIFO1排队
	CAN_FilterInitStructure.CAN_FilterActivation = ENABLE;//过滤器激活，激活指定的过滤器
