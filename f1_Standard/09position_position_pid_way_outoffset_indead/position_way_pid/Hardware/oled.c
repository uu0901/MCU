...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SCL_GPIO_PORT, OLED_SCL_GPIO_PIN, (BitAction)BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	GPIO_WriteBit(OLED_SDA_GPIO_PORT, OLED_SDA_GPIO_PIN, (BitAction)BitValue);
...
void oledShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')	//遍历字符串
	{
		...
		if (CharLength == 1)	//如果是单字节字符
		{
			...
			oledShowChar(X + XOffset, Y, SingleChar[0], FontSize);
			XOffset += FontSize;
		}
		else					//否则，即多字节字符
		{
			...
			if (FontSize == OLED_8X16)		//给定字体为8*16点阵
			{
				...
				oledShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data);
				XOffset += 16;
			}
			else if (FontSize == OLED_6X8)	//给定字体为6*8点阵
			{
				/*空间不足，此位置显示'?'*/
				oledShowChar(X + XOffset, Y, '?', OLED_6X8);
				XOffset += OLED_6X8;
			}
		}
	}
...
void oledShowNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位							
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		oledShowChar(X + i * FontSize, Y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void oledShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	if (Number >= 0)						//数字大于等于0
	{
		oledShowChar(X, Y, '+', FontSize);	//显示+号
	...
	for (i = 0; i < Length; i++)			//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		oledShowChar(X + (i + 1) * FontSize, Y, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void oledShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位
	{
		...
		if (SingleNumber < 10)			//单个数字小于10
		{
			...
			/*+ '0' 可将数字转换为字符格式*/
			oledShowChar(X + i * FontSize, Y, SingleNumber + '0', FontSize);
		}
		else							//单个数字大于10
		{
			/*调用OLED_ShowChar函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			oledShowChar(X + i * FontSize, Y, SingleNumber - 10 + 'A', FontSize);
		}
	}
}
...
void oledShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位	
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		oledShowChar(X + i * FontSize, Y, Number / OLED_Pow(2, Length - i - 1) % 2 + '0', FontSize);
	}
}
...
void oledShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	/*显示整数部分*/
	oledShowNum(X + FontSize, Y, IntNum, IntLength, FontSize);
	
	/*显示小数点*/
	oledShowChar(X + (IntLength + 1) * FontSize, Y, '.', FontSize);
	
	/*显示小数部分*/
	oledShowNum(X + (IntLength + 2) * FontSize, Y, FraNum, FraLength, FontSize);
}
...
void oledPrintf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
	...
	va_end(arg);							//结束变量arg
	oledShowString(X, Y, String, FontSize);//OLED显示字符数组（字符串）
}
...
void oledDrawLine(int16_t X0, int16_t Y0, int16_t X1, int16_t Y1)
{
	...
	else				//斜线
	{
		...
		/*画起始点，同时判断标志位，将坐标换回来*/
		if (yflag && xyflag){oledDrawPoint(y, -x);}
		else if (yflag)		{oledDrawPoint(x, -y);}
		else if (xyflag)	{oledDrawPoint(y, x);}
...
void oledDrawRectangle(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
	...
	if (!IsFilled)		//指定矩形不填充
	{
		...
		{
			oledDrawPoint(i, Y);
			oledDrawPoint(i, Y + Height - 1);
		}
		/*遍历左右Y坐标，画矩形左右两条线*/
		for (i = Y; i < Y + Height; i ++)
		{
			oledDrawPoint(X, i);
			oledDrawPoint(X + Width - 1, i);
		}
	}
	else				//指定矩形填充
	{
	...
	else					//指定三角形填充
	{
		...
		for (i = minx; i <= maxx; i ++)
		{
			...
			for (j = miny; j <= maxy; j ++)
			{
				...
				/*如果在，则画点，如果不在，则不做处理*/
				if (OLED_pnpoly(3, vx, vy, i, j)) {oledDrawPoint(i, j);}
			}
		}
	}
}
...
void oledDrawArc(int16_t X, int16_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, uint8_t IsFilled)
{
	...
	if (OLED_IsInAngle(x, y, StartAngle, EndAngle))	{oledDrawPoint(X + x, Y + y);}
	if (OLED_IsInAngle(-x, -y, StartAngle, EndAngle)) {oledDrawPoint(X - x, Y - y);}
	if (OLED_IsInAngle(y, x, StartAngle, EndAngle)) {oledDrawPoint(X + y, Y + x);}
	if (OLED_IsInAngle(-y, -x, StartAngle, EndAngle)) {oledDrawPoint(X - y, Y - x);}
	...
	if (IsFilled)	//指定圆弧填充
	{
		...
		for (j = -y; j < y; j ++)
		{
			...
			if (OLED_IsInAngle(0, j, StartAngle, EndAngle)) {oledDrawPoint(X, Y + j);}
		}
	}
	...
	while (x < y)		//遍历X轴的每个点
	{
		...
		if (IsFilled)	//指定圆弧填充
		{
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(x, j, StartAngle, EndAngle)) {oledDrawPoint(X + x, Y + j);}
				if (OLED_IsInAngle(-x, j, StartAngle, EndAngle)) {oledDrawPoint(X - x, Y + j);}
			}
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(-y, j, StartAngle, EndAngle)) {oledDrawPoint(X - y, Y + j);}
				if (OLED_IsInAngle(y, j, StartAngle, EndAngle)) {oledDrawPoint(X + y, Y + j);}
			}
		}
	}
