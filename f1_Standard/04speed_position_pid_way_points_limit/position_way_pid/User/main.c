...
/*
要解决的问题：如果执行器因为卡住、断电、损坏等原因不能消除误差，则误差积分会无限制加大，进而达到深度饱和状态，
此时PID控制器会持续输出最大的调控力，即使后续执行器恢复正常，PID控制器在短时间内也会维持最大的调控力，直到误差积分从深度饱和状态退出
...
int main(void)
{
	...
	*/	
	...
	while (1)
	{
		...
		Target = adcGetValue(4) / 4095.0 * 300 - 150;//目标
...
void TIM1_UP_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) == SET)
	{
		...
		if (Count >= 40)
		{
			...
			ErrorInt += Error0;
			//积分限幅-
			//确定积分项输出占总输出的最大比例，最简单就是积分项输出最大和总输出一致，也就是积分限幅和输出限幅一致。单独对误差积分的限幅就是，积分限幅除以ki，也就是outLimt/ki就是误差积分限幅：100/0.2 = 500
