...
#define NRF24L01_RX_PACKET_WIDTH		10							//接收通道0数据包宽度，范围：1~32字节
//uint8_t NRF24L01_RxPacket[NRF24L01_RX_PACKET_WIDTH];				//接收数据包




NRF_ADDR_T NRF24L01_TxPacket= {0x00,{0x11,0x22,0x33,0x44,0x55}};
NRF_ADDR_T NRF24L01_RxPacket= {0x00, {0x11,0x22,0x33,0x44,0x55}};
NRF_ADDR_T NRF24L01_DummyPacket= {0x00,{0xff,0xff,0xff,0xff,0xff}};
//// 使用时：
//tx_addr.cmd = 0xA0;
//MySPI_DMA_Send((uint8_t*)&tx_addr, 6);
/**
  * 提示：设备A和设备B进行通信
...
//以下指令操作发送和接收之前都会手动操作CS引脚，这是NRF24L01+的通信要求
/*指令实现*********************/
...
void NRF24L01_ReadRegs(uint8_t RegAddress, uint8_t *DataArray, uint8_t Count)
{
	...
	/*交换发送一个字节，通信开始的第一个字节为指令码，读寄存器（低5位为寄存器地址）*/
//	NRF24L01_SPI_SwapByte(NRF24L01_R_REGISTER | RegAddress);
//	NRF24L01_TxPacket.cmd = NRF24L01_R_REGISTER | RegAddress;
	
	// 使用DMA接收数据
	if(Count > DMA_COUNT_OPEN) 
	{
//		MySPI_DMA_Receive(NRF24L01_TxPacket, Count + 1);
	} 
	else 
	{
		MySPI_ReceiveMultiBytes(DataArray, Count + 1);
	}
...
/**
  ...
  * 返 回 值：无
  */
void NRF24L01_WriteRegs(uint8_t RegAddress, NRF_ADDR_T  *DataArray, uint8_t Count)
{
	...
	/*交换发送一个字节，通信开始的第一个字节为指令码，写寄存器（低5位为寄存器地址）*/
//	NRF24L01_SPI_SwapByte(NRF24L01_W_REGISTER | RegAddress);
	DataArray -> cmd = NRF24L01_R_REGISTER | RegAddress;
	// 使用DMA发送数据
	if(Count > DMA_COUNT_OPEN) 
	{
//		MySPI_DMA_Send(NRF24L01_TxPacket, Count + 1);
		MySPI_DMA_Send((uint8_t*)DataArray, sizeof(NRF_ADDR_T));
	} 
	else
	{
		MySPI_SwapByte(DataArray->cmd);
		MySPI_SendMultiBytes(DataArray.packet, Count + 1);
	}
...
//	NRF24L01_SPI_SwapByte(NRF24L01_R_RX_PAYLOAD);
	DataArray[0] = NRF24L01_R_RX_PAYLOAD;
	if(Count > DMA_COUNT_OPEN) 
	{
//		MySPI_DMA_Receive(DataArray, Count + 1);
	} 
	else 
	{
		MySPI_ReceiveMultiBytes(DataArray, Count + 1);
...
//	NRF24L01_SPI_SwapByte(NRF24L01_W_TX_PAYLOAD);
	DataArray[0] = NRF24L01_W_TX_PAYLOAD;
	// 使用DMA发送数据
	if(Count > DMA_COUNT_OPEN) 
	{
		MySPI_DMA_Send(DataArray, Count + 1);
	} 
	else
	{
		MySPI_SendMultiBytes(DataArray, Count + 1);
	}
...
uint8_t NRF24L01_Send(void)
{
	...
	NRF24L01_WriteTxPayload(NRF24L01_TxPacket, sizeof(NRF_ADDR_T));
...
uint8_t NRF24L01_Receive(void)
{
	...
	else if ((Status & 0x40) == 0x40)	//状态寄存器位6（RX_DR）为1
	{
		...
		NRF24L01_ReadRxPayload(NRF24L01_RxPacket, sizeof(NRF_ADDR_T));
