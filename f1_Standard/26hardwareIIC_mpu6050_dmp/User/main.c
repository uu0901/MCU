...
//	ledInit();
	...
	MPU6050_DMPInit();		//MPU6050初始化
	/*显示ID号*/
	oledShowString(0, 0, "ID:", OLED_8X16);		//显示静态字符串
	ID = MPU6050_GetID();						//获取MPU6050的ID号
	oledShowHexNum(24, 0, ID, 2, OLED_8X16);    //OLED显示ID号
	oledUpdate();
	delayMs(3000);
	oledClear();	
...
//		oledShowSignedNum(0, 16, AX, 5, OLED_8X16);					//OLED显示数据
//		oledShowSignedNum(0, 32, AY, 5, OLED_8X16);
//		oledShowSignedNum(0, 48, AZ, 5, OLED_8X16);
//		oledShowSignedNum(56, 16, GX, 5, OLED_8X16);
//		oledShowSignedNum(56, 32, GY, 5, OLED_8X16);
//		oledShowSignedNum(56, 48, GZ, 5, OLED_8X16);

		
		oledPrintf(0, 16, OLED_8X16, "Pitch:%+06.1f", Pitch);
		oledPrintf(0, 32, OLED_8X16, "Roll :%+06.1f", Roll);
		oledPrintf(0, 48, OLED_8X16, "Yaw  :%+06.1f", Yaw);
	}
}
